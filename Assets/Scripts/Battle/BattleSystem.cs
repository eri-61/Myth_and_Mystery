...
public class BattleSystem : MonoBehaviour
{
    ...
    public void OnItemButton()
    {
        ...
        Items selected = InventoryManager.instance.GetSelectedItem(true);
        UseItemInBattle(selected);
    }

    private void UseItemInBattle(Items item)
    {
        if (state != Battlestate.PLAYERTURN) return;
        if (item == null) return;

        // Compare by name or some unique identifier
        if (correctItem != null && item.name.Equals(correctItem.itemName, StringComparison.OrdinalIgnoreCase))
        {
